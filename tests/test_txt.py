...
from tests.paths import DIR_WITH_TEST_FILES, PATH_TO_TEST_OUTPUT_TXT_FILE
from tinybear.txt import (
...
def test_move_line(file_stem, line_number_to_cut, line_number_to_insert_before):
    dir_with_test_files = DIR_WITH_TEST_FILES / "move_line"
...
def test_move_line_throws_exception_with_wrong_string():
    with pytest.raises(TypeError, match='Please pass an integer or the string "END"'):
        move_line(
            file=DIR_WITH_TEST_FILES / "move_line" / "tc9-11.txt",
...
def test_read_plain_text_from_file():
    for file in DIR_WITH_TEST_FILES.glob("read_plain_text*.txt"):
        logging.info(f"TEST: reading plain content from {file=}")
